...
WatchMe v8は、ユーザーの心理状態と行動ログを可視化・分析するライフログツールです。EC2上のVault APIからのデータを基にモバイルファースト設計のダッシュボードで日々の活動を客観的に表示し、管理者による通知管理機能も備えています。
...
## ⚠️【重要】ユーザーアカウント仕様・セキュリティ状況

### 🔓 現在の認証状況（テスト環境レベル）

**重大な注意事項**: 現在のシステムは**認証機能が一切存在しない**テスト環境レベルの実装です。
...
- **パスワード認証**: なし
- **セッション管理**: なし
- **アクセス方法**: URLパラメータ (`?userId=user123`) でユーザー指定

#### セキュリティリスク
- 任意のユーザーIDでアクセス可能：`/dashboard?userId=任意のID`
- 管理画面への直接アクセス：`/admin`（認証なし）
- 他人のデータへの無制限アクセス
- ブラウザ履歴にユーザーIDが残存
...
├── user456 (佐藤あやか) - 子アカウント (8歳)
└── user789 (佐藤みなと) - 子アカウント (5歳)
user1748359410612 (田中花子) - テスト用マスターアカウント
```
...
#### アクセス方法
- **URL**: `/admin`（認証なし）
- **環境制限**: 開発環境でのみ完全な管理機能が利用可能

#### 主要機能
1. **ユーザー管理**
   - 全ユーザー一覧表示・編集・削除
   - 新規ユーザー作成（タイムスタンプベースID生成）
   - 親子アカウント関係管理

2. **データ管理**
   - JSONファイル直接アップロード
   - EC2 Vault APIからの一括データ取得
   - ファイルシステムへの直接書き込み
...
#### 管理画面の重大なセキュリティ問題
- **認証機能の完全欠如**: URLアクセスのみで管理画面利用可能
- **データアクセス制御なし**: 全ユーザーデータへの無制限アクセス
- **任意データ注入可能**: JSON形式での任意データアップロード
- **操作ログなし**: 管理操作の記録・追跡不可
- **API保護なし**: 管理API呼び出しに認証不要
...
#### 問題点
- 親子関係のアクセス制御が不十分
- アバター表示の不整合（プロフィール画像パスの問題）
- 状態管理の不備（リロード時に情報が失われる）
- セッション管理なし

### 📁 データ保存方式

- **ユーザーデータ**: `data_accounts/users.json`
- **個別ログ**: `data_accounts/{userId}/logs/{date}.json`
- **プロフィール画像**: `/avatars/avatar-{userId}.png`
- **通知データ**: `data_accounts/{userId}/notifications.json`

### 🚧 今後の改修予定

このシステムは現在**テスト環境レベル**です。今後以下の改修を予定しています：
...
**現在のシステムは開発・テスト目的での利用に限定してください。**
- プロダクション環境での運用は推奨されません
- 機密データの保存は避けてください
- 外部公開時は必ず認証機能を実装してください
